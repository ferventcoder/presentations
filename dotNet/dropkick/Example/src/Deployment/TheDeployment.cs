...
namespace Deployment
{
    public class TheDeployment : Deployment<TheDeployment, DeploymentSettings>
    {
        ...
        public TheDeployment()
        {
            Define(settings =>
            {
                ...
                DeploymentStepsFor(Db,
                    s =>
                    {
                        s.RoundhousE()
                            ...
                            .WithRepositoryPath(settings.RepositoryPath)
                ...
                DeploymentStepsFor(Web,
                                   s =>
                                   {
                                       s.CopyDirectory(@"..\_PublishedWebSites\WebApp").To(@"{{WebsitePath}}").DeleteDestinationBeforeDeploying();
                ...
                DeploymentStepsFor(Host,
                                   s =>
                                   {
                                       var serviceName = "DUDE.{{Environment}}";
                                       s.WinService(serviceName).Stop();

                                       s.CopyDirectory(@"..\_PublishedApplications\ConsoleApp").To(@"{{HostServicePath}}").DeleteDestinationBeforeDeploying();

                                       s.CopyFile(@"..\environment.files\{{Environment}}\{{Environment}}.ConsoleApp.exe.config").ToDirectory(@"{{HostServicePath}}").RenameTo(@"ConsoleApp.exe.config");
                                       ...
                                       s.WinService(serviceName).Create().WithCredentials(settings.ServiceUserName, settings.ServiceUserPassword).WithDisplayName("DUDE AWESOME ({{Environment}})").WithServicePath(@"{{HostServicePath}}\ConsoleApp.exe").
        ...
        public static Role Console { get; set; }
